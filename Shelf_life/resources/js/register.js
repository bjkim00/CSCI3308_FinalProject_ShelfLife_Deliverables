  ...
  for (var i = 0; i < numItems.value; i++) {
    document.getElementById('menuItem').innerHTML +=
      ...
      <div class="form-group row textcenter">
        ...
        <div class="col-sm-1">
          <button class="btn btn-primary btn-block" type="button" onclick="ingredients()">Enter</button>
        </div>
      </div>
      `
    for (var n = 0; n < numIngredients.value; n++) {
      document.getElementById('menuItem').innerHTML +=
        `
          <div id="ingridz${n}">
...
function ingredients() {
  for (var j = 0; j < numIngredients.value; j++) {
    document.getElementById(`ingridz${j}`).innerHTML +=
      `
      <div class="form-group row">
